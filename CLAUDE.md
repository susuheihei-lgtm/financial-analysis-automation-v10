...
v5/
  ...
  yfinance_parser.py        # yfinance + SEC EDGAR + IR BANK データ取得
  irbank_parser.py          # IR BANK CSVパーサー（日本株年次5年分）＋HTMLスクレイピング（四半期）
  excel_parser.py           # Excelファイルパーサー（.xls/.xlsx、日本語対応）
  templates/index.html      # シングルページアプリ（HTML/CSS/JS）
  static/css/, static/js/   # 静的アセット
  data/                     # サンプルExcelデータ / IR BANKキャッシュ（年次CSV・四半期JSON）
...
- **日本株（年次）**: IR BANK CSV（最大5年分、自動DL）→ yfinance（gross_profit・total_debt等の不足フィールド補完）
- **日本株（四半期）**: IR BANK HTMLスクレイピング（Q1/Q2/Q3、最大12期）→ yfinance（BS補完）
- **EDINET削除済み**: 複雑なXBRLパース・企業ごとのタグ差異による欠損多発のため廃止
